...
# --- Baseline 1: Gradient Ascent (unchanged) ---
...
# --- Baseline 2: Synaptic Dampening (unchanged) ---
...
# --- New Baseline 3: Fisher Forgetting ---
        ...
        with torch.no_grad():
            for name, param in model.named_parameters():
                if name in fisher_dict:
                    param -= lambda_fisher * fisher_dict[name] * param  # Dampen based on Fisher info
    else:
        print("No samples for forget class found; skipping Fisher Forgetting.")

# Apply Fisher Forgetting
# ResNet18
resnet.load_state_dict(torch.load("models/ResNet18_speaker_classifier.pth", map_location=device)['model_state_dict'])  # Reset
begin = time.time()
fisher_forgetting_unlearn(resnet, train_loader, forget_class=0, device=device, lambda_fisher=0.1)
end = time.time()
print(f"Time Taken : Fisher Forgetting : Resnet : {end-begin}")
torch.save({'model_state_dict': resnet.state_dict()}, "models/ff_unlearned_ResNet18.pth")
...
print(f"Time Taken : Fisher Forgetting : ViT : {end-begin}")
torch.save({'model_state_dict': vit.state_dict()}, "models/ff_unlearned_ViT-Tiny.pth")

# --- New Baseline 4: Negative Gradient ---
def negative_gradient_unlearn(model, train_loader, forget_class, device, epochs=5):
    ...
    for epoch in range(epochs):
        ...
        for spectrograms, labels in train_loader:
            ...
            with autocast():
                outputs = model(spectrograms_forget)
                loss = -criterion(outputs, labels_forget)  # Negative loss to "unlearn"
            scaler.scale(loss).backward()
            scaler.step(optimizer)
            scaler.update()
            running_loss += loss.item()
        if running_loss != 0.0:  # Only print if there was forget data
...
print(f"Time Taken : Negative Gradient : Resnet : {end-begin}")
torch.save({'model_state_dict': resnet.state_dict()}, "models/ng_unlearned_ResNet18.pth")
...
print(f"Time Taken : Negative Gradient : ViT : {end-begin}")
torch.save({'model_state_dict': vit.state_dict()}, "models/ng_unlearned_ViT-Tiny.pth")

print("Unlearning completed for all methods: Custom Quantum, Gradient Ascent, Synaptic Dampening, Fisher Forgetting, Negative Gradient.")