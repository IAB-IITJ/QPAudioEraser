...
# Initialize models
resnet = models.resnet18(pretrained=True)  # Training from scratch
...
# --- Unlearning Phase ---
# Custom Quantum-Inspired Unlearning
Fo = 0  # Forget class
...
class QuantumLoss(nn.Module):
    def __init__(self, forget_class, lambda_param=1.0, num_classes=100):
        ...
        self.uniform_label = torch.ones(num_classes, device=device) / num_classes
    ...
    def forward(self, logits, targets):
        ...
        uniform_loss = torch.nn.functional.cross_entropy(
            logits, self.uniform_label.expand_as(logits), reduction='none') * mask
        loss = (1 - mask) * ce + self.lambda_param * uniform_loss
        return loss.mean()

criterion_quantum = QuantumLoss(forget_class=0, lambda_param=1.0, num_classes=100)
...
begin = time.time()
train_unlearning(resnet, train_loader, criterion_quantum, device)
end = time.time()
print(f'Custom Model Time : Resnet : {end - begin:.2f}')
K = 100
alpha = 0.5
...
begin = time.time()
train_unlearning(vit, train_loader, criterion_quantum, device)
end = time.time()
print(f"Custom Model Time : ViT : {end - begin:.2f}")
...
print("Training and unlearning completed for all methods: Custom Quantum, Gradient Ascent, Synaptic Dampening, Fisher Forgetting, Negative Gradient.")